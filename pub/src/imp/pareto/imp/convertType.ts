...
export function convertType<Annotation>(
    $: ts.TType<Annotation>,
    logMessage: ($: string, context: Annotation) => void
): t.TParetoType | undefined {
                ...
                return pl.cc($.type[1], ($) => {
                    ...
                    if ($.type !== null) {
                        ...
                        if ($.type === null) {
                            ...
                            const x = convertType($.type, logMessage)
                        }
                        //entries.add(name,)
                    }
                })
    ...
    switch ($.type[0]) {
        ...
        case "typeLiteral":
            return pl.cc($.type[1], ($) => {
                const entries = pc.createUnsafeDictionaryBuilder<t.TParetoTypeSignature>()
                $.forEach(($) => {
        ...
        case "typeReference":
            return pl.cc($.type[1], ($) => {
                ...
                //                 }
                //             }
                //         })return pl.ccreturn pl.cc                    //         })return pl.ccreturn pl.cc

                //     default: return pl.au($.identification[0])
                // }
                return ["reference", {}]
            })
